...
struct BWMainTabsView: View {
    ...
    private func saveCategory(_ draft: BWCategoryDraft) async -> Bool {
        switch draft.mode {
            ...
            case .edit(let originalCategory):
                ...
                guard let plannedAmount = Int64(exactly: draft.plannedAmount) else {
                    return false
                }
                category.amountPlanned = BWMoneyAmount(value: plannedAmount)
    ...
    private func saveTransaction(_ draft: BWTransactionDraft) async -> Bool {
        switch draft.mode {
            ...
            case .edit(let item):
                guard let amount = Int64(exactly: draft.amount) else {
                    return false
                }
