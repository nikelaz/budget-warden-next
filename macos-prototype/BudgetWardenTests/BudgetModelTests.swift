...
final class BudgetModelTests: XCTestCase {
    ...
    func testBudgetRowIdentityUsesURLNotBudgetID() {
        let first = BudgetRow(
        ...
        XCTAssertEqual(first.budgetID, second.budgetID)
        XCTAssertNotEqual(first.id, second.id)
    }

    func testCategoryTypeUsesStableJSONValues() {
        XCTAssertEqual(BudgetCategoryType.income.rawValue, "income")
        XCTAssertEqual(BudgetCategoryType.expenses.rawValue, "expenses")
        XCTAssertEqual(BudgetCategoryType.savings.rawValue, "savings")
        XCTAssertEqual(BudgetCategoryType.debt.rawValue, "debt")
    }

    func testDateFormattedDatePadsMonthAndDay() {
        let date = BWDate(year: 2026, month: 5, day: 7)

        XCTAssertEqual(date.formattedDate, "2026-05-07")
    }

    func testStringUsesFallbackForMissingTitle() {
        XCTAssertEqual("".swiftString(default: "Untitled"), "Untitled")
        XCTAssertEqual("Budget".swiftString(default: "Untitled"), "Budget")
    }
}
